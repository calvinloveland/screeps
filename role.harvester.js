...
var roleHarvester = {
    ...
    run: function (creep) {
        ...
        if ((creep.carry.energy < creep.carryCapacity && creep.memory.harvesting === true) || creep.carry.energy === 0) {
            ...
            if (targets.length > 0) {
                // if there are empty targets only include those targests
                lowestEnergy = _.min(targets, function (target) { return target.energy; });
                targets = targets.filter((target) => {
                    return target.energy - 50 < lowestEnergy.energy;
                });
